...
videos can induce reliable, time-locked responses across participants, suggesting
that familiar videos contain shared response structure aligned to playback time
\cite{hasson2004intersubject,kauppi2010isc,dmochowski2012correlated}.
When training participants and a new viewer watch the same familiar video on
an aligned playback timeline, their matched labels permit a group affect
trajectory indexed by video identity and within-video time. A valid predictor
must reconstruct this trajectory inside each training split so that held-out
labels cannot enter the estimate.
...
\end{equation}
Here, \(N\) denotes the number of evaluated one-second
participant--video--time samples, and \(i\) indexes each sample tuple
\((s,v,t)\).
...
Figure~\ref{fig:method-overview} summarizes this leakage-controlled,
source-explicit pipeline.

\begin{figure*}[!t]
  \centering
  \includegraphics[width=\textwidth]{method_overview.pdf}
  \caption{Method overview. (A) A new viewer contributes EEG--fNIRS at second
  \(t\) of a familiar video. (B) The video--time prior branch (blue) aggregates
  training-participant labels via a fold-wise cross-participant median and
  radius-three within-video smoothing to yield \(\bar p_{v,t}\); held-out labels
  are excluded. The EEG--fNIRS branch (orange) extracts one-second features from
  \(t-1\), \(t\), and \(t+1\), applies graph encoders and bidirectional cross-modal
  attention, and regresses \(m_{s,v,t}\). (C) Prior-dominating fixed fusion with
  valence and arousal prior weights \(w=[0.99,0.92]\) produces
  \(\hat y_{s,v,t}\).}
  \Description{A three-panel overview shows a new viewer watching a familiar
  video, the leakage-controlled video--time prior and participant-specific
  EEG--fNIRS branches, and their fixed fusion into a predicted valence--arousal
  trajectory.}
...
Models trained on participant-grouped folds minimize mean squared error on
these scaled targets plus \(0.01\) times a symmetric contrastive alignment loss
\cite{chen2020simclr}. Predictions are transformed back using
\(254\hat{\tilde y}+1\) and clipped to \([1,255]\) before MAE evaluation. Hereafter, \(m_{s,v,t}\) denotes this transformed and clipped prediction on
the original \([1,255]\) scale. The alignment term regularizes paired EEG and
fNIRS representations from the same sample without dominating the regression
objective. All evaluated EEG--fNIRS checkpoints use a dropout rate of \(0.7\).
Models trained on participant-grouped folds use AdamW optimization and early
...
are matched to predictions only for offline evaluation. The external EEG--fNIRS
estimate averages predictions from six checkpoints. For the final checkpoint,
EEG and fNIRS features are standardized using statistics computed from all 24
development participants. Each of the five participant-grouped fold checkpoints uses
feature-standardization statistics computed from its training participants.
Fixed fusion combines this ensemble with a video--time prior built from all 24
...
identical participant--video--time samples. We also computed overall MSE as a
secondary external diagnostic; MAE remains the primary endpoint. For external
heterogeneity analysis, paired errors are also aggregated by participant, by
familiar video,
and by ten equal bins of normalized within-video time. For each video--time
cell, MAE equally pools the valence and arousal absolute errors of all
one-second samples assigned to that cell. These grouped summaries and the
predicted-versus-observed densities are descriptive because the external cohort
is small and adjacent one-second targets are temporally correlated. The 6,143
one-second observations are therefore not treated as independent replicates.
For the internal evaluation, only aggregate component metrics are available,
so paired sample-level residual analysis is performed on the external cohort.
...
Figure~\ref{fig:external-prediction-quality} complements the scalar external
scores with a descriptive view of calibration and error structure. Fixed fusion
achieved valence MAE of 25.20 and arousal MAE of 30.25. Both densities showed
prediction-range compression toward the middle of the scale, with the effect
more pronounced for arousal. These plots identify score ranges that contributed
to error, but they do not treat the temporally correlated second-level
observations as independent evidence.

\begin{figure*}[!t]
  \centering
  \includegraphics[width=\textwidth]{external_prediction_quality.pdf}
  \caption{External fixed-fusion predicted-versus-observed densities. (a,b)
  Hexbin densities compare predictions with observed valence and arousal,
  respectively, for 6,143 one-second samples. The dashed diagonal marks exact
  prediction; dotted horizontal and vertical lines mark the neutral score of
  128. Colour gives sample count on a logarithmic scale, and panel titles report
  MAE on the original \([1,255]\) scale. Both panels show prediction-range
  compression toward the middle of the scale, particularly for arousal.}
  \Description{Two hexbin plots compare fixed-fusion predictions with observed
  valence and arousal. Dashed diagonals mark exact prediction, dotted lines mark
  the neutral score of 128, and predicted arousal is concentrated mid-range.}
  \label{fig:external-prediction-quality}
\end{figure*}
...
Using unrounded overall MAE, the video--time prior accounted for 99.73\% and
97.89\% of the total internal and external reductions from EEG--fNIRS to fixed
fusion, respectively. Each percentage was calculated as the MAE improvement
from EEG--fNIRS to the prior divided by the corresponding improvement from
EEG--fNIRS to fusion. The fixed fusion remained best,
...
The contribution of within-video time also varied over playback.
Figure~\ref{fig:external-decomposition}d provides a one-dimensional marginal
after pooling errors across videos within each normalized time bin. In the first
bin, video identity reached 45.53 MAE and the video--time prior reached 13.40.
This advantage narrowed over time, and video identity was slightly better in
each of the final five bins. The aggregate temporal gain was therefore
concentrated in specific phases rather than distributed uniformly.

Figure~\ref{fig:external-video-time-mae} restores the two-dimensional
video--time structure hidden by that marginal summary. Overall MAE was 28.04
for the video--time prior and 27.72 for fixed fusion. Across the 150 cells,
fusion had lower MAE in 76, equal MAE in 7, and higher MAE in 67. The two
methods shared 9 of their 10 highest-error cells. Their main hotspot occurred
in the latter half of V1. Thus, the small aggregate fusion gain reflected many
local changes of both signs rather than a uniform reduction across the grid.

\begin{figure*}[!t]
  \centering
  \includegraphics[width=\textwidth]{external_video_time_mae.pdf}
  \caption{External video--time MAE. (a) Video--time prior and (b) fixed fusion
  across 15 familiar videos and ten equal bins of normalized playback time.
  Each cell equally pools valence and arousal absolute errors over all
  one-second samples assigned to that video--time cell. Both panels use the
  same fixed 0--65 MAE scale, with deep blue denoting lower error and
  orange--red denoting higher error. Overall MAE was 28.04 for the prior and
  27.72 for fusion.}
  \Description{Two side-by-side heatmaps compare video--time prior and fixed
  fusion MAE across 15 videos and ten normalized playback-time bins. Deep blue
  denotes lower error and orange--red denotes higher error; the strongest
  hotspot occurs in the latter half of video V1.}
  \label{fig:external-video-time-mae}
\end{figure*}
