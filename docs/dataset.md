...
本研究使用两个互不重叠的被试集合：24 名开发集被试用于训练与五折来源分解，4 名外部被试用于独立评估。两部分数据都包含同步 EEG、fNIRS、静息基线和 1 Hz 效价—唤醒度标注。
...
所有被试观看相同且时间对齐的视频，因此仅按被试划分不会移除视频标识和时间坐标。论文据此在每个外层折内仅使用训练被试构建视频—时间先验，并在完全相同的留出样本上比较该先验与 EEG--fNIRS 生理分支。

外部数据用于验证来源排序是否在新的观看者群体上复现，并保留逐样本成对预测，从而分析：
...
当前协议支持关于“新被试观看这 15 个已见视频”的结论，但不能证明模型能够泛化到未见视频。若研究目标是刺激无关的生理解码，应进一步采用视频留出或被试×刺激交叉留出设计。
